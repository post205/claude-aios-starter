---
name: audit
description: Use when someone asks for an AIOS audit, asks to score their setup against The Stack, or says "is my AIOS working" / "audit my setup" / "find gaps in my AIOS". Produces a Stack scoreboard (Context, Connections, Capabilities, Cadence) with top-3 fixes ranked by leverage.
---

## What this skill does

Runs the **Stack Audit** on the current Claude Code project. Reads (never writes) the project's operating manual, memory, skills, agents, connections, decisions, and references. Scores each of the four Stack layers out of 25. Surfaces strengths and the top 3 leverage-weighted gaps with concrete next-step commands.
...
## The Stack (scored 25 each = 100 total)

Full definitions, build order, and per-layer failure modes: `references/the-stack.md`.

| Layer | Test |
|---|---|
| **Context** | Knows the business — identity, team, voice, decisions, references |
| **Connections** | Reaches the user's tools and data — integrations, CLIs, scripts, data sources |
