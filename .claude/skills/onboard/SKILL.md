...
**The wow moment:** at the end, suggest the closing prompt *"Try this — ask me: what should I focus on this week?"* The user runs it once. That's the wow. There's no `/today` skill to save — the prompt itself plants Move 1 of The Shift (Question — "to what extent could AI do this?") for them to internalize.
...
That closing question seeds Move 1 of The Shift (Question) before `/level-up` formally introduces the framework on Day 14.
...
7. **Read-only on `references/the-shift.md`.** It already ships in the kit. Don't overwrite.
...
> *The Question language used in the closing screen comes from `references/the-shift.md`.*
